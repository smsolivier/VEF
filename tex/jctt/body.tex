...
Note that the Eddington factor depends only upon the angular shape of the angular flux, and not its magnitude.  This drift-diffusion equation is derived by first taking the first two angular moments of Eq.~\ref{eq:sn}: 
	...
	\end{equation}
	\end{subequations}
% added isotropic part since the MMS solution uses a non-isotropic source 
where $J(x)$ is the current \change{and the fixed source has been assumed to be isotropic}. Then Eq.~\ref{eq:first} is solved for $J(x)$, and this expression is then substituted into
Eq.~\ref{eq:zero}. 
